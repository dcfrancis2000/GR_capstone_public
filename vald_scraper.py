...
def main():
    ...
    bb_df = ath.loc[ath.team_name.isin(["Former Athletes","Men's Basketball"])]
    bb_id = list(bb_df.athlete_id.unique())
    
    # starting 5 player IDs
    bb_id_top5 = list(bb_df[bb_df["name"].isin(['Brendan Wenzel','Caden Powell','Sam Griffin','Akuel Kot','Cam Manyawu'])]['athlete_id'])
    print(bb_id_top5)
    ...
    for i in range(0,5):
        ...
        data['athlete_id'] = id
        data['name'] = list(bb_df['name'][bb_df['athlete_id'] == id])[0]
    ...
    vald_full.to_csv(r"C:\Users\dcfra\GR_capstone\data\vald_scraped.csv")
